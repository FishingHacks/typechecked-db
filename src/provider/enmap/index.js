...
exports.default = {
    ...
    async create(name, obj) {
        if (typeof obj.id !== 'number')
            throw new Error('Expected obj.id to be of type number. Found ' + typeof obj.id);
...
function ensureUnique(name, id) {
    let entries = db.get(name);
    if (!(entries instanceof Array))
        entries = [];
    return !entries.find((el) => el.id === id);
}
function runQueryOnArr(arr, query) {
    let matches = [];
    for (const v of arr) {
        if (runQueryOnAttr(v, query))
            matches.push(v);
    }
    return matches;
}
function runQueryOnArrOne(arr, query) {
    for (const v of arr) {
        if (runQueryOnAttr(v, query))
            return v;
    }
...
function runQueryOnArrAndDelete(arr, query) {
    let isFound = false;
    return arr.filter((el) => (isFound = isFound || !runQueryOnArr(el, query)));
}
function runQueryOnArrAndUpdate(arr, query, newValue) {
    let isSet = false;
    let value = undefined;
    return [
        arr.map((el) => {
            if (!isSet && runQueryOnAttr(el, query)) {
                ...
                if (typeof value.id !== 'number')
                    value.id = el.id;
                if (value.id !== el.id)
                    throw new Error('The id of the updated object has to be the same as the id of the existent object!');
...
function runQueryOnAttr(attr, query) {
    let isMatching = true;
    for (const [k, v] of Object.entries(query)) {
        if (typeof query[k] === 'object') {
            ...
            if (q.gt && typeof v === 'number' && v > q.gt)
                isMatching = true;
            else {
                isMatching = false;
                break;
            }
            if (q.lt && typeof v === 'number' && v < q.lt)
                isMatching = true;
            else {
                isMatching = false;
                break;
